#!/usr/bin/env python3
"""
scan_suspicious.py — detect masquerading / double-extension files.

Scans a folder tree for files that look harmless but aren't:

1. Double-extension masquerades — ``photo.png.exe``, ``invoice.pdf.scr``.
   A file whose LAST extension is executable-ish (exe, scr, bat, cmd, ...)
   while an earlier extension claims to be an image / document.  On
   Windows with "hide known file extensions" enabled, ``photo.png.exe``
   is displayed as ``photo.png`` — a classic malware trick.

2. Magic-byte mismatches — the file's real header bytes don't match the
   extension it shows.  A ``photo.png`` that actually starts with ``MZ``
   is a renamed Windows executable; an ``invoice.pdf`` containing a ZIP
   header is something other than what it claims to be.

Pure standard library — no dependencies.  Works with any Python 3.

Usage:
    python scan_suspicious.py [folder] [--json] [--no-recursive]

Exit code: 0 = nothing suspicious, 1 = at least one finding.
"""
...
def magic_matches(header: bytes, ext: str) -> bool:
    ...
    for offset, sig in probes:
        probe = sig
        if header[offset : offset + len(probe)] == probe:
            return True
    return False
...
def scan(root: Path, recursive: bool) -> list[dict]:
    results: list[dict] = []
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Find double-extension masquerades and files whose "
        "magic bytes don't match their displayed extension."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="scan only the given folder, not subfolders",
    )
    ...
    results = scan(root, recursive=not args.no_recursive)

    if args.json:
        print(json.dumps(results, indent=2))
    else:
        if not results:
            print(f"No suspicious files found in {root}.")
        else:
            for r in results:
                print(f"[{r['severity']}] {r['path']}")
                for note in r["findings"]:
                    print(f"          - {note}")
            print(
                f"\n{len(results)} suspicious file(s) found. "
                "Enable 'Show file extensions' in Explorer "
                "(View -> File name extensions) and inspect them manually."
            )
